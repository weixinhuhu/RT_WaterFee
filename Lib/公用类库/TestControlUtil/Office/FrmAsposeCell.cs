...
namespace TestControlUtil
{
    public partial class FrmAsposeCell : Form
    {
        ...
        private void btnBindExcel_Click(object sender, EventArgs e)
        {
            ...
            string savedFile = WHC.Framework.ControlUtil.AsposeExcelTools.ExportWithDataSource(templateFile, "testAdvice.xls", dictSource);
        ...
        private void btnReplaceContent_Click(object sender, EventArgs e)
        {
            ...
            string templateFile = Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "Templates/Advice.xls");
            string savedFile = WHC.Framework.ControlUtil.AsposeExcelTools.ExportWithReplace(templateFile, "testAdvice.xls", dictSource);
            if (!string.IsNullOrEmpty(savedFile))
            {
                if (MessageUtil.ShowYesNoAndTips("导出成功，是否打开文件？") == System.Windows.Forms.DialogResult.Yes)
                {
