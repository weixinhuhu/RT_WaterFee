...
namespace WHC.Framework.ControlUtil
{
    ...
    public abstract class AbstractBaseDAL<T> where T : BaseEntity, new()
    {
        ...
        public AbstractBaseDAL(string tableName, string primaryKey, bool _isAutoIncrement = false)
            : this()
        {
        ...
        /// <returns></returns>
        public virtual bool Insert3(T obj, DbTransaction trans = null)
        {
            ...
            return Insert3(hash, trans);
        }
        public virtual bool Insert3(Hashtable recordField, DbTransaction trans)
        {
            return this.Insert3(recordField, tableName, trans);
        }
        public virtual bool Insert3(Hashtable recordField, string targetTable, DbTransaction trans)
        {
        ...
        public virtual bool InsertNotLog3(T obj, DbTransaction trans = null)
        {
            ArgumentValidation.CheckForNullReference(obj, "传入的对象obj为空");
        ...
        public virtual int Insert2(Hashtable recordField, string targetTable, DbTransaction trans)
        {
            ...
            fields = fields.Trim(',');//除去前后的逗号
            vals = vals.Trim(',');//除去前后的逗号
            string sql = string.Format("INSERT INTO {0} ({1}) VALUES ({2});select @@IDENTITY", targetTable, fields, vals);
        ...
        public virtual bool PrivateUpdate(object id, Hashtable recordField, string targetTable, DbTransaction trans)
        {
            try
            {
                ...
                foreach (string field in recordField.Keys)
                {
                    if (this.primaryKey.ToUpper() == field.ToUpper() && isAutoIncrement) continue;//跳过自增主键,即有标识的主键
        ...
        public virtual DataTable SqlTable(string sql, DbParameter[] parameters, DbTransaction trans = null)
        {
            ...
            else
            {
                ...
                //string constring = System.Configuration.ConfigurationManager.ConnectionStrings["mysql"].ConnectionString;
                //MySqlConnection con = new MySqlConnection(constring);
                //con.Open();
                //MySqlDataAdapter Mydt = new MySqlDataAdapter(sql, con);
                //DataSet ds = new DataSet();
                //Mydt.Fill(ds, "settings");
                //dt = ds.Tables[0];
        ...
        /// <param name="trans">事务对象</param>
        /// <returns>执行成功返回true或false</returns>
        public virtual bool InsertNotLog(T obj, DbTransaction trans = null)
        {
            ArgumentValidation.CheckForNullReference(obj, "传入的对象obj为空");
        ...
        /// <param name="trans">事务对象</param>
        /// <returns>执行成功返回<c>true</c>，否则为<c>false</c>。</returns>
        public virtual bool UpdateNotLog(T obj, object primaryKeyValue, DbTransaction trans = null)
        {
            ArgumentValidation.CheckForNullReference(obj, "传入的对象obj为空");
        ...
        public virtual T FindSingle(string condition, string orderBy, IDbDataParameter[] paramList, DbTransaction trans = null)
        {
            ...
            {
                LogTextHelper.Debug(string.Format("检测出SQL注入的恶意数据, {0}", orderBy));
                throw new Exception("检测出SQL注入的恶意数据");
            }
        ...
        public List<T> GetListByView(string viewName, string condition, DbTransaction trans = null)
        {
            if (HasInjectionData(condition))
            {
                LogTextHelper.Debug(string.Format("检测出SQL注入的恶意数据, {0}", condition));
        ...
        public bool StoreProcExecute(string storeProcName, Hashtable inParameters = null, Hashtable outParameters = null, DbTransaction trans = null)
        {
            Database db = CreateDatabase();
            DbCommand command = db.GetStoredProcCommand(storeProcName);
            //参数传入
        ...
        public List<T> StoreProcToList(string storeProcName, object outParameters, DbTransaction trans = null)
        {
        ...
        public virtual void SetStoreParameters(Database db, DbCommand command, Hashtable inParameters = null, Hashtable outParameters = null)
        {
            #region 参数传入
            //传入输入参数
        ...
        /// <param name="command">DbCommand对象</param>
        /// <param name="outParameters">输出参数的哈希表</param>
        public virtual void EditOutParameters(Database db, DbCommand command, Hashtable outParameters = null)
        {
            #region 获取输出参数的值
            if (outParameters != null)
            {
        ...
        /// <param name="intParameters"></param>
        /// <param name="outParameters"></param>
        /// <param name="trans"></param>
        /// <returns></returns>
        public DataTable StorePorcToDataTable(string storeProcName, List<DbParameter> intParameters, List<DbParameter> outParameters, DbTransaction trans = null)
        {
            Database db = CreateDatabase();
            DbCommand command = db.GetStoredProcCommand(storeProcName);
            ...
            DataTable result = null;
            result = trans != null ? db.ExecuteDataSet(command, trans).Tables[0] : db.ExecuteDataSet(command).Tables[0];
            if (result != null)
            {
                result.TableName = "tableName";//增加一个表名称，防止WCF方式因为TableName为空出错
            }
            foreach (var outParameter in outParameters)
            {
                outParameter.Value = db.GetParameterValue(command, outParameter.ParameterName);
            }
            return result;
        }
        ...
        /// <param name="trans"></param>
        /// <returns></returns>
        public DataTable StorePorcToDataTable(Database database, string storeProcName, List<DbParameter> intParameters, List<DbParameter> outParameters, DbTransaction trans = null)
        {
            ...
            string sSQL = "SELECT * FROM a_rcv_tmp";
            ...
            for (int i = 0; i < command.Parameters.Count; i++)
            {
                ...
                {
                    dt.ExtendedProperties.Add(command.Parameters[i].ParameterName, command.Parameters[i].Value);
                }
            }
        ...
        /// <param name="outParameters">输入参数</param>
        /// <param name="trans"></param>
        /// <param name="intParameters">输出参数</param>
        /// <returns></returns>
        public bool StoreProcToResult(string storeProcName, List<DbParameter> intParameters, List<DbParameter> outParameters, DbTransaction trans = null)
        {
            Database db = CreateDatabase();
            DbCommand command = db.GetStoredProcCommand(storeProcName);
            foreach (var intParameter in intParameters)
            ...
            int result = trans != null ? db.ExecuteNonQuery(command, trans) : db.ExecuteNonQuery(command);
            foreach (var outParameter in outParameters)
            {
                outParameter.Value = db.GetParameterValue(command, outParameter.ParameterName);
            }
        ...
        /// <param name="intParameters">输出参数</param>
        /// <returns></returns>
        public bool ExecStoreProc(string storeProcName, List<SqlParameter> Parameters = null, DbTransaction trans = null)
        {
            Database db = CreateDatabase();
            DbCommand command = db.GetStoredProcCommand(storeProcName);
            foreach (var intParameter in Parameters)
            {
                command.Parameters.Add(intParameter);
            }

            int result = trans != null ? db.ExecuteNonQuery(command, trans) : db.ExecuteNonQuery(command);
            foreach (var outParameter in Parameters)
            {
                if (outParameter.Direction == ParameterDirection.Output)
                    outParameter.Value = db.GetParameterValue(command, outParameter.ParameterName);
            }
            return result > 0;
        }
        ...
        /// <param name="intParameters">输出参数</param>
        /// <returns></returns>
        public DataSet ExecStoreProcToDataSet(string storeProcName, List<SqlParameter> Parameters = null, DbTransaction trans = null)
        {
            Database db = CreateDatabase();
            DbCommand command = db.GetStoredProcCommand(storeProcName);
            foreach (var intParameter in Parameters)
            ...
            DataSet ds = trans != null ? db.ExecuteDataSet(command, trans) : db.ExecuteDataSet(command);
            foreach (var outParameter in Parameters)
            {
                if (outParameter.Direction == ParameterDirection.Output)
                    outParameter.Value = db.GetParameterValue(command, outParameter.ParameterName);
